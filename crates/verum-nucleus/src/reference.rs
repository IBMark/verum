...
detector_table! {
    ...
    StaleSuppression {
        category: "Diagnostics",
        summary: "A verum:ignore comment that suppresses nothing",
        detects: "A `verum:ignore` (or `verum:ignore[Kind,...]`) comment with no matching \
                  finding on its own line or the line directly below - either nothing is \
                  flagged there any more, or the bracket filter names kinds that do not \
                  occur there. Detected only in files that currently have findings.",
        why: "A suppression that matches nothing is either a note about an issue that was \
              fixed (delete it) or a trap: it sits in the code ready to silently swallow \
              the next real finding introduced at that spot. Low severity, unscored, and \
              never gating - but visible, so suppressions cannot rot.",
        lang: "python",
        bad: r#"
        ...
        good: r#"
            rows = db.execute(PREPARED_QUERY, (user_id,))
        "#,
        suppress: "It cannot itself be suppressed or baselined - that would defeat its \
                   purpose. Delete the stale comment, or fix its kind list so it matches \
