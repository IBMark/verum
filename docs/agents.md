...
```
verum gate <PATH>
verum gate <PATH> --baseline old-report.json
```

Flags:

- `--baseline <FILE>` - compare against a previous `verum report --format json`
  output or a `verum baseline` file, matched by stable fingerprint. The gate
  then fails ONLY on new findings: a new `Critical` beyond
  `deploy_gate.max_critical_issues`, or any new `High`. Score thresholds do
  not apply in this mode. A missing or corrupt file is an error (exit `1`
  with an `Error:` message), never an empty baseline.
- `--show-suppressed` - list the findings inline `verum:ignore` comments
...
- `--baseline <FILE>` - partition findings against a previous JSON report or a
  `verum baseline` file, matched by stable fingerprint. Adds a `baseline`
  section to the JSON report and a `## Baseline` section to markdown. A
  missing or corrupt file is an error, never an empty comparison.
- `--show-suppressed` - include the findings inline `verum:ignore` comments
  removed (a `suppressed` array in JSON, a listing in markdown).
...
| `suppressed_count` | int | Findings removed by inline `verum:ignore` comments this run. |
| `suppressed` | array | Only with `--show-suppressed`: the removed findings, same shape as `findings`. |
| `baseline` | object | Only with `--baseline`: `{ new[], resolved[], existing_count }` - `new` and `resolved` are fingerprint lists, sorted. |
...
| `fingerprint` | string | 16-hex stable identity: hashed from kind, repo-relative path, symbol name, digit-stripped message, occurrence index. Survives line drift and repo relocation; what `--baseline` matches on. |
...
```
verum baseline <PATH>
verum baseline <PATH> --out findings-2024.json
```

Flags:

- `--out <OUT>` - write the baseline here instead of
  `<PATH>/verum.baseline.json`, for feeding `gate --baseline` /
  `report --baseline` explicitly.

Exit code: always `0`.

Writes a minimal, diff-friendly snapshot: one `{fingerprint, kind, severity}`
entry per finding, sorted by fingerprint - no paths, messages, or line numbers
to churn when code moves. Commit it. The fingerprint is hashed from the kind,
the repo-relative path, the symbol name, the digit-stripped message, and an
occurrence index, so a finding survives line drift and the repo living at a
different absolute path, but a genuinely new finding does not match. Advisory
kinds - daisychains, Rust perf insights, `CrateApiMisuse`, `ParseFailure`,
`StaleSuppression` - are excluded from both the snapshot and the comparison.
Baseline files from earlier releases (`version: 1`, textual fingerprints) are
still honored by the auto-loaded ratchet.

Once `verum.baseline.json` exists, `verum gate` stops applying score
thresholds and fails only on new `Critical` or `High` findings. Do not edit or
delete it to make a gate pass; regenerate it deliberately when you have fixed
a batch.
...
{"version":2,"count":3,"findings":[{"fingerprint":"b8305d855bd207e3","kind":"DeadFunction","severity":"Medium"}, ...]}
$ verum gate ./demo; echo "exit=$?"
...
A comment on a finding's line, or the line directly above it, waives that
finding at the source instead of in the baseline:

```
# verum:ignore                       every kind on this/the next line
// verum:ignore[WeakCrypto] reason   only the listed kinds, free-text reason
```

The token must be inside a comment for the file's language family (`//` and
`/*` for C-family languages, `#` for Python/shell/config, `--` for SQL/Lua).
Suppressed findings leave `findings`, the score, and the gate, but the report
always carries `suppressed_count`, and `--show-suppressed` lists them. A
`verum:ignore` that matches nothing becomes a Low `StaleSuppression` finding
so dead suppressions are visible instead of lying in wait.
